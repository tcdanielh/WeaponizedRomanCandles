...
public class ScreenWriter : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        foreach (Ejecta e in es) Debug.Log(e.pos);
    ...
    //    float rho_p = 2000;    // projectile density [kg/m^3]
    //    Vector3 v_a = new Vector3(0,0,0);  // air velocity [m/s]
    //    float m_s = 2; // inert structural mass [kg]
    //    
    //
    //    // options to change burst radius, flight time, accuracy
    //    float m_e = 5; // explosive charge mass [kg]
    //    float m_f = 10;    // launch charge mass [kg]
    //    float t_e = 5; // time of explsion [sec]
    //    float dt = 0.001f;  // time step [sec]
    //
    //    float m_i = 4 / 3 * Mathf.PI * Mathf.Pow((float)R_p, 3) * rho_p;  // mass of ejecta 
    ...
    //    }
    //    int num_steps_ascent = Mathf.FloorToInt(t_e / dt); 
    ...
    //    Vector3 psi_tot = new Vector3(0,0,0);
